...
static void test_maps_pixel_to_cell() {
    ...
    auto cell = mapper.pixel_to_cell(30, 30);
    EXPECT_TRUE(cell.has_value());
    EXPECT_EQ(cell.value(), Position(0, 0));

    cell = mapper.pixel_to_cell(90, 30);
    EXPECT_TRUE(cell.has_value());
    EXPECT_EQ(cell.value(), Position(0, 1));

    cell = mapper.pixel_to_cell(30, 90);
    EXPECT_TRUE(cell.has_value());
...
static void test_rejects_outside_board() {
    ...
    EXPECT_FALSE(mapper.pixel_to_cell(210, 30).has_value());
    EXPECT_FALSE(mapper.pixel_to_cell(30, 210).has_value());
}

static void test_uses_board_dimensions_not_hardcoded_8x8() {
    BoardMapper mapper(4, 5);

    auto cell = mapper.pixel_to_cell(270, 210);
    EXPECT_TRUE(cell.has_value());
    EXPECT_EQ(cell.value(), Position(3, 4));

    EXPECT_FALSE(mapper.pixel_to_cell(330, 210).has_value());
}
