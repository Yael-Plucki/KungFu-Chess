...
void WebSocketServer::enqueue_disconnect(const std::string& connection_id) {
    std::lock_guard<std::mutex> lock(inbound_mutex_);
    disconnect_queue_.push(connection_id);
}
...
bool WebSocketServer::pop_disconnect(std::string& connection_id) {
    std::lock_guard<std::mutex> lock(inbound_mutex_);
    if (disconnect_queue_.empty()) {
        return false;
    }

    connection_id = std::move(disconnect_queue_.front());
